...
class IntegerValidatorTest extends Specification {
    ...
    def "check that all numbers in the list are integers"() {
        ...
        then:
        this.list.collect({ number -> validator.isAnInteger(number) }).inject(true) { result, i -> result && i }
    }
    ...
    def "check that all numbers in the list are positive integers"() {
        ...
        then:
        this.list.collect({ number -> validator.isAPositiveInteger(number) }).inject(true) { result, i -> result && i }
    }
    ...
    def "check that all numbers in the list are negative integers"() {
        ...
        then:
        this.list.collect({ number -> validator.isANegativeInteger(number) }).inject(true) { result, i -> result && i }
    }
    ...
    def "check that all values in the list notIntegerNumbers are not integer numbers"() {
        ...
        then:
        list.collect({ number -> !validator.isAnInteger(number) }).inject(true) { result, i -> result && i }
    }
